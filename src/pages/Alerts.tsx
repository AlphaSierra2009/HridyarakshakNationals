...
export default function AlertsPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      <h2 className="text-2xl font-semibold mb-4 gradient-text neon-glow">Emergency Alerts</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          {alerts.map((a) => (
            <Card key={a.id} className={`p-3 mb-2 cursor-pointer hover-lift transition-all ${selected?.id === a.id ? 'border-emergency emergency-pulse card-glass' : 'card-glass'}`} onClick={() => setSelected(a)}>
              <div className="flex justify-between items-center">
                <div>
                  <div className="font-semibold neon-glow">{a.alert_type} {a.is_test ? '(test)' : ''}</div>
        ...
        <div className="md:col-span-2">
          {selected ? (
            <Card className="p-4 card-glass">
              <h3 className="font-semibold neon-glow">Alert details</h3>
              ...
              <div className="mt-3">
                {selected.reading_snapshot ? (
                  <>
                    <Line
                      data={{ labels: (selected.reading_snapshot as number[]).map((_, i) => i), datasets: [{ data: selected.reading_snapshot, borderColor: 'hsl(var(--emergency))', fill: false, pointRadius: 0 }] }}
