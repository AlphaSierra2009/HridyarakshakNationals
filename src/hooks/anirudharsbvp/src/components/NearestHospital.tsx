...
// Overpass API query builder to find nearby hospitals (amenity=hospital, healthcare=clinic fallback)
const overpassQuery = (lat: number, lon: number, radius = 5000) => {
  // Requests nodes and ways; returns center for way if available
...
export default function NearestHospital({ location }: NearestHospitalProps) {
  ...
  async function fetchNearby(lat: number, lon: number, searchRadius = 3000) {
    ...
    try {
      ...
      withDist.sort((a,b) => (a.distanceMeters || 1e9) - (b.distanceMeters || 1e9));
  ...
  const openNavigation = (destLat: number, destLon: number) => {
    // Open Google Maps (preferred). On iOS, this will prompt to open Apple Maps if configured.
    // Use universal google maps url which works on desktop & mobile
  ...
  return (
    <Card className="p-4 overflow-hidden border-2 bg-card/60">
      ...
      {!location ? (
        <div className="text-sm text-muted-foreground">Waiting for location…</div>
      ) : (
        <>
          <div className="flex items-center gap-2 mb-2 text-xs text-muted-foreground">
            <Clock className="h-4 w-4" /> Search radius:
            <select value={radius} onChange={(e) => setRadius(Number(e.target.value))} className="ml-1 bg-transparent border rounded px-2 py-1 text-sm">
          ...
          {loading ? (
            <div className="text-sm text-muted-foreground">Searching nearby hospitals…</div>
          ) : hospitals.length === 0 ? (
            <div className="text-sm text-muted-foreground">No hospitals found within {formatMeters(radius)}.</div>
          ) : (
            <div className="space-y-2">
              {hospitals.slice(0, 6).map((h) => (
                <div key={h.id} className="flex items-center justify-between p-2 bg-zinc-900 rounded">
                  <div>
                    <div className="font-medium text-sm">{h.name}</div>
                    <div className="text-xs text-muted-foreground">{h.address ?? formatMeters(h.distanceMeters)}</div>
                    <div className="text-xs text-muted-foreground mt-1">Distance: {formatMeters(h.distanceMeters)}</div>
                  </div>

                  <div className="flex flex-col items-end gap-2">
                    <Button size="sm" onClick={() => openNavigation(h.lat, h.lon)} className="whitespace-nowrap">
                      <Navigation className="h-3 w-3 mr-2 inline-block" />
                      Navigate
                    </Button>
                    <a target="_blank" rel="noreferrer" href={`https://www.google.com/maps/search/?api=1&query=${h.lat},${h.lon}`} className="text-xs text-muted-foreground underline">
                      View on map
                    </a>
                  </div>
                </div>
